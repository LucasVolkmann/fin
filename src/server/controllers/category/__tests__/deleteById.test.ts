...
describe('Test [controller category deleteById]', () => {
  ...
  it('should return BAD_REQUEST status if request has not a token', async () => {
    await request(app)
      .delete(CategoryRoutesEnum.CATEGORY_ID.replace(':id', '1'))
      .expect(StatusCodes.BAD_REQUEST);
  });
  it('should return BAD REQUEST when id path param is not a number', async () => {
    await request(app)
      .delete(CategoryRoutesEnum.CATEGORY_ID.replace(':id', 'Non number'))
      .auth(validToken, {type: 'bearer'})
      .expect(StatusCodes.BAD_REQUEST);
  });
  it('should return BAD REQUEST when id path param is not a integer', async () => {
    await request(app)
      .delete(CategoryRoutesEnum.CATEGORY_ID.replace(':id', '1.5'))
      .auth(validToken, {type: 'bearer'})
      .expect(StatusCodes.BAD_REQUEST);
  });
